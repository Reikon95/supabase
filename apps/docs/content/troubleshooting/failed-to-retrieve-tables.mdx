...
First, you can verify if this is isolated to your project of if it's a platform issue by viewing our [status page](https://status.supabase.com/).
...
- Restart the instance from the project settings in the dashboard once you are confident there are no large ongoing queries (if there are, it may crash again). You can do that on [this page](https://supabase.com/dashboard/project/_/settings/general). If it crashes again, re-review the logs to understand what may be casuing the issue.

After that:

- If this was intentional activity (often it's a case of uploading, or reading, a lot of test data), and it needs to be done again, batch requests into smaller chunks and reduce concurrency, and implementation pagination for select queries. Check out our [query optimisation docs](https://supabase.com/docs/guides/database/query-optimization) for more information around general ideas here.
...
- Consider your table and your query structure - if your tables are very "wide" (lots of columns) or have complicated data types within them (jsonb, base64 in particular), it may be worth revisiting your architecture.
- Continue to monitor your project's [query performance tab](https://supabase.com/dashboard/project/_/observability/query-performance) and [enable index advisor](https://supabase.com/docs/guides/database/extensions/index_advisor) if you haven't already - especially if there are a lot of select queries.
- If after monitoring your changes you still do not notice improvements, consider upgrading compute if you think this level of activity is going to be regular. It will give you more memory overhead to process tasks like this. You can view all compute offerings [here](https://supabase.com/dashboard/project/_/settings/compute-and-disk).

If you want to effectively monitor your project's performance minute by minute, you can use the [Metrics API](https://supabase.com/docs/guides/telemetry/metrics).
